...
public final class StartApplicationMsgTypeSupportSA implements TypeSupport {
	...
	public StartApplicationMsgTypeSupportSA() {
		StartApplicationMsgSA tmp = new StartApplicationMsgSA();
	...
	public DataReader create_datareader(Subscriber sub, TopicDescription td,
		...
		return new StartApplicationMsgDataReaderSA(sub, td, jni_dr);
	}

	public DataWriter create_datawriter(Publisher pub, Topic topic,
			SWIGTYPE_p__DataWriter jni_dw) {
		return new StartApplicationMsgDataWriterSA(pub, topic, jni_dw);
	}

	// marshal variants
	public int marshall(ByteBuffer out_stream, StartApplicationMsgSA src) {
		int size = 0;
		if (out_stream == null) {
	...
	public int marshal_size_core(ByteBuffer out_stream, int size, StartApplicationMsgSA src) {
	    ...
	    size = (size+3) & 0xfffffffc;// align 4
	    size += 4; // length
	    if (src.applicationName == null) size += 1;
	    else {
	      try {
	        byte[] sbytes = src.applicationName.getBytes("UTF-8");
	        size += sbytes.length + 1;
	      } catch(Exception e) {
	        size += 1;
	      }
	    ...
	    size = (size+3) & 0xfffffffc;// align 4
	    size += 4; // length
	    if (src.applicationType == null) size += 1;
	    else {
	      try {
	        byte[] sbytes = src.applicationType.getBytes("UTF-8");
	        size += sbytes.length + 1;
	      } catch(Exception e) {
	        size += 1;
	      }
	...
	public int marshal_core(ByteBuffer out_stream, int offset, StartApplicationMsgSA src) {
		// src.eventName
		while ((offset & 0x03) != 0) {
			offset++;
			out_stream.put((byte) 0x00);
	...
	public int marshall_key(ByteBuffer out_stream, StartApplicationMsgSA src) {
		int size = 0;
		if (out_stream == null) {
			size += 4; // for CDR 'header'
		} else {
	...
	public int marshall_key_hash(ByteBuffer out_stream, StartApplicationMsgSA src) {
		int size = 0;
		if (out_stream == null) {
		} else {
			int offset = 0;
	...
	// unmarshal variants
	public int unmarshall(StartApplicationMsgSA t, ByteBuffer data, int s) {
		int offset = 0;
	...
	public int unmarshal_core(StartApplicationMsgSA t, ByteBuffer data, int offset, int s) {
		// t.eventName
		{
			while ((offset & 0x03) != 0) {
				offset++;
	    ...
	    // t.applicationName
	    {
	      ...
	      data.get(); // skip trailing null
	      try {;
	        t.applicationName   = new String(sbytes, "UTF-8");
	      } catch(java.io.UnsupportedEncodingException e) {
	        t.applicationName   = new String(); }
	      offset += 4 + slen + 1;
	    }
	    ...
	    // t.applicationType
	    {
	      ...
	      data.get(); // skip trailing null
	      try {;
	        t.applicationType   = new String(sbytes, "UTF-8");
	      } catch(java.io.UnsupportedEncodingException e) {
	        t.applicationType   = new String(); }
	      offset += 4 + slen + 1;
	    }
	...
	public int unmarshall_key(StartApplicationMsgSA t, ByteBuffer data, int s) {
		int offset = 0;
		data.get(); // skip the first byte
		byte encoding = data.get(); // data encoding CDR / ENDIAN
		data.getShort(); // unused flags (2 bytes)
	...
	public int unmarshall_key_hash(StartApplicationMsgSA t, ByteBuffer data, int s) {
		int offset = 0;
		data.order(ByteOrder.BIG_ENDIAN);
		return 0;
	}
	...
	// <type> operations
	public StartApplicationMsgSA alloc() {
		return new StartApplicationMsgSA();
	}

	public void clear(StartApplicationMsgSA instance) {
		instance.clear();
	}

	public void destroy(StartApplicationMsgSA instance) { /* noop */
	}

	public void copy(StartApplicationMsgSA to, StartApplicationMsgSA from) {
		to.copy(from);
	}
