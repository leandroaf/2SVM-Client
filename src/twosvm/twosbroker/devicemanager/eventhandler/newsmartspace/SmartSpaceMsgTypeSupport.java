...
public final class SmartSpaceMsgTypeSupport implements TypeSupport {
	...
	public SmartSpaceMsgTypeSupport() {
		SmartSpaceMsg tmp = new SmartSpaceMsg();
	...
	public DataReader create_datareader(Subscriber sub, TopicDescription td,
		...
		return new SmartSpaceMsgDataReader(sub, td, jni_dr);
	}

	public DataWriter create_datawriter(Publisher pub, Topic topic,
			SWIGTYPE_p__DataWriter jni_dw) {
		return new SmartSpaceMsgDataWriter(pub, topic, jni_dw);
	}

	// marshal variants
	public int marshall(ByteBuffer out_stream, SmartSpaceMsg src) {
		int size = 0;
		if (out_stream == null) {
	...
	public int marshal_size_core(ByteBuffer out_stream, int size, SmartSpaceMsg src) {
		// src.eventName
	    ...
	    size = (size+3) & 0xfffffffc;// align 4
	    size += 4; // length
	    if (src.location == null) size += 1;
	    else {
	      try {
	        byte[] sbytes = src.location.getBytes("UTF-8");
	        size += sbytes.length + 1;
	      } catch(Exception e) {
	        size += 1;
	      }
	...
	public int marshal_core(ByteBuffer out_stream, int offset, SmartSpaceMsg src) {
		// src.eventName
		while ((offset & 0x03) != 0) {
			offset++;
			out_stream.put((byte) 0x00);
	...
	public int marshall_key(ByteBuffer out_stream, SmartSpaceMsg src) {
		int size = 0;
		if (out_stream == null) {
			size += 4; // for CDR 'header'
		} else {
	...
	public int marshall_key_hash(ByteBuffer out_stream, SmartSpaceMsg src) {
		int size = 0;
		if (out_stream == null) {
		} else {
			int offset = 0;
	...
	// unmarshal variants
	public int unmarshall(SmartSpaceMsg t, ByteBuffer data, int s) {
		int offset = 0;
	...
	public int unmarshal_core(SmartSpaceMsg t, ByteBuffer data, int offset, int s) {
		// t.eventName
		{
			while ((offset & 0x03) != 0) {
				offset++;
	    ...
	    // t.eventDescription
	    {
	      ...
	      data.get(); // skip trailing null
	      try {;
	        t.location   = new String(sbytes, "UTF-8");
	      } catch(java.io.UnsupportedEncodingException e) {
	...
	public int unmarshall_key(SmartSpaceMsg t, ByteBuffer data, int s) {
		int offset = 0;
		data.get(); // skip the first byte
		byte encoding = data.get(); // data encoding CDR / ENDIAN
		data.getShort(); // unused flags (2 bytes)
	...
	public int unmarshall_key_hash(SmartSpaceMsg t, ByteBuffer data, int s) {
		int offset = 0;
		data.order(ByteOrder.BIG_ENDIAN);
		return 0;
	}
	...
	// <type> operations
	public SmartSpaceMsg alloc() {
		return new SmartSpaceMsg();
	}

	public void clear(SmartSpaceMsg instance) {
		instance.clear();
	}

	public void destroy(SmartSpaceMsg instance) { /* noop */
	}

	public void copy(SmartSpaceMsg to, SmartSpaceMsg from) {
		to.copy(from);
	}
