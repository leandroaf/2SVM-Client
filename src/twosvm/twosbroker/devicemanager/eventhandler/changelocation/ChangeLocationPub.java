...
public class ChangeLocationPub {

	public void changeLocation(String eventName, String newLocation) {
		
		System.out.println("Mudanca de contexto-------------------------");
		...
		Topic top = dp.create_topic("contextChangeTopic", ts.get_type_name(), null, // default
		// 		Topic top = dp.create_topic("contextChangeTopic", ts.get_type_name(), null, // default
																// qos
				null, 0); // no listener
		...
		ChangeLocationMsgDataWriter dw = (ChangeLocationMsgDataWriter) pub.create_datawriter(
				top, dw_qos, dw_listener, 0);

		int i = 1;
		
		ChangeLocationMsg data = new ChangeLocationMsg();
		
		data.eventName = new String(eventName);
		data.newLocation = new String(newLocation);
