package twosvm.uctwosmiddleware.localeventhandler.contextchange;

import twosvm.model.instance.contextchange.ContextChange;
import twosvm.model.instance.user.User;
import twosvm.uctwosmiddleware.localeventhandler.contextchange.ContextChangeMsg;
import twosvm.uctwosmiddleware.localeventhandler.contextchange.ContextChangeMsgDataWriter;
import twosvm.uctwosmiddleware.localeventhandler.contextchange.ContextChangeMsgTypeSupport;
import twosvm.uctwosmiddleware.localmodelinterpreter.LocalModelInterpreter;

import com.toc.coredx.DDS.*;

public class ContextChangePub {
	...
	public void newEvent(ContextChange cChange) throws Exception {
		...
		ContextChangeMsgDataWriter dw = (ContextChangeMsgDataWriter) pub
				.create_datawriter(top, dw_qos, dw_listener, 0);

		ContextChangeMsg data = new ContextChangeMsg();
		...
		data.appID = new String(user.applicationID()); // envia todos os IDs das
														// aplicacoes, separados
														// por virgula
		data.appTypeSS = new String(user.appTypeSS()); // envia todos os tipos
														// de aplicacoes,
														// separados por virgula
		data.appNameSS = new String(user.applicationNameSS()); // envia todos os
																// nomes das
																// aplicacoes,
																// separados por
																// virgula
